...
export function LeaderboardView() {
  ...
  async function loadDaily(selectedDate: string) {
    ...
    setError('');
    try {
      const res = await fetchLeaderboard(selectedDate, 25, 0, 'dead-letters');
  ...
  async function loadAggregated(p: 'weekly' | 'monthly', selectedDate: string) {
    ...
    setError('');
    try {
      const res = await fetchAggregatedLeaderboard(p, selectedDate, 25, 0, 'dead-letters');
    ...
    <div className="view">
      ...
      {period === 'daily' && (
        <div className="leaderboard-controls">
          <label htmlFor="dl-date-picker" className="leaderboard-controls__label">
            Date:
          </label>
          <input
            id="dl-date-picker"
